...
const fetchWeatherData = async(cityName) => {
    try {
        ...
        clearData();
        document.querySelector("#todayForecast_City").textContent = currentDayData.name+", "+currentDayData.sys.country;
...
const clearData = () => {
    ...
    while(tableTodayForecast.firstChild) tableTodayForecast.removeChild(tableTodayForecast.firstChild);

    let futureTableForecast = document.querySelector("#futureForecast_Table");
    while(futureTableForecast.firstChild) futureTableForecast.removeChild(futureTableForecast.firstChild);
...
const drillAndFetchData = (data,attribute) => {
    ...
    if(weatherOptions.hasOwnProperty(attribute)) return weatherOptions[attribute]();
};

const transformDate = (data, requiredDateType) => {
    if(requiredDateType){
        return new Date(data * 1000).toLocaleString("en-us",{month:"long",day:"numeric",year:"numeric"});
    } else {
        return new Date(data * 1000).toLocaleString("en-us",{hour:"numeric",minute:"numeric"});
    }
};
...
const populateFutureForecast = async(latitutde, longitude) => {
    ...
    let fetchedData = await fetch(`https://api.openweathermap.org/data/2.5/onecall?lat=${latitutde}&lon=${longitude}&appid=${Your_App_Key_Here}&exclude=current,minutely,hourly&units=Metric`,{mode:'cors'});
    ...
    document.querySelector("#futureForecast_Heading").textContent = "Forecast for next week";
...
const populateTodayForecast = (data) => {
    ...
    document.querySelector("#todayForecast_Date").textContent = "Today's Forecast - "+drillAndFetchData(data,"Date");

    document.querySelector("#todayForecast_Type").src = drillAndFetchData(data,"Weather Type");
